Before we start training, we need to check the training case's signal toggle-power coverage rate.
1. Coefficient of Variation (CV) = std / mean , means the relative power fluctuation. It is a value >=0, the larger the better. 
2. the std and mean of the power. 
3. signal toggle rate = popcount(state[t] xor state[t-1])
4. active columns

Also refer to the absolute value of the std. and the mean. of the power. Because if they are both very small. Then This module may not need to be modeled, the power fluctuation can come from noise.
1. The standard deviation of the power (target). It measures how large does the power fluctuates around its mean. If the std of a power is 3mW, and the power mean is 5mW, then its already very large power fluctuations. 
2. The mean of the power. 

If we find some modules have low coverage rage, we need to add direct cases to improve its coverage.
