...
def plot_trace(
    ...
    """Overlay of ground truth and prediction over the first ``max_cycles`` rows.

    One row is one prediction: a cycle at ``data.window_size = 1``, otherwise a
    window of that many cycles (``xlabel`` names the unit on the plot).
    """
    import matplotlib
    ...
    ax.plot(np.arange(n), yhat[:n], color=_ORANGE, linewidth=0.9, label="prediction")
