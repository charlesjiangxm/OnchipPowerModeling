...
## Language
1. Write in synthesizable System Verilog (`.sv`) and simulate with `vcs -sverilog`. only the following dialects are allowed: `always_ff` / `always_comb` (avoid using `always`), `logic`, `$clog2`.
2. Use `integer` for procedural loop variables inside `always` blocks. `generate`-`for` loops still require `genvar` — a `genvar` cannot be replaced by `integer`.
3. For module I/O ports, do not use multi-dimensional / unpacked arrays. Flatten to a single packed vector (e.g. `[N*W -1:0]`) and slice with `+:` (`vec[i*W +: W]`).
4. Do not use `function` or `task`. Factor helper / reusable logic into its own module in a separate `.v` file and instantiate it (`U_*`).
