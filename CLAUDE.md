...
- `src/` — one script per model family, all sharing the same loader, feature selector, and split scaffolding:
  ...
  - `models/ft_transformer.py` — self-contained PyTorch FT-Transformer (`NumericalFeatureTokenizer`, `TransformerBlock`, `FTTransformer`), training loop (`train_ft_transformer`), inference helpers (`predict`, `extract_attention`), and the pipeline-facing `FTTransformerModel` wrapper, all in one module.
  - `torch_utils.py` — model-agnostic Torch/numpy helpers shared across wrappers: `default_device(pref)` (CUDA > MPS > CPU), `describe_device`, `Standardizer` (float64 z-score for X and y), and the `to_tensor` thin wrapper. Imported by `preprocess.py`, `models/mlp.py`, and `models/ft_transformer.py`.
