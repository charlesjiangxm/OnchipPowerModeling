################################################################################
# Design Compiler synthesis for feed_forward_network_registered
# (FT-Transformer position-wise FFN: Linear1 -> ReLU -> Linear2, int8 Q1.7).
#
# Mirrors dc_layer_norm.tcl but targets the FFN block. The RTL uses
# SystemVerilog always_ff/always_comb, so it is analyzed as -format sverilog.
# The activation is ReLU (replacing the model's GELU); there is no scalar
# constant to derive, so only D_TOKEN/D_FFN/DATA_WIDTH/FRAC_BITS are passed to
# elaborate.
#
# Run:  ./run_dc_feed_forward_network.csh -mode syn [-batch_dir <dir>] [param overrides]
################################################################################
...
set TOP_MODULE_NAME feed_forward_network_registered
...
set D_FFN        [get_env_or_default D_FFN        64]
...
if {[info exists ::env(BATCH_DIR)] && $::env(BATCH_DIR) ne ""} {
  ...
  set BATCH_DIR [file join ${SYN_ROOT} "batch_feed_forward_network_${date_hour}"]
}
...
set rtl_files [list \
  ...
  ${RTL_ROOT}/feed_forward_network_registered.v \
]

# RTL uses SystemVerilog always_ff/always_comb -> analyze as sverilog.
analyze -format sverilog $rtl_files

set ELAB_PARAMS "D_TOKEN=${D_TOKEN},D_FFN=${D_FFN},DATA_WIDTH=${DATA_WIDTH},FRAC_BITS=${FRAC_BITS}"
